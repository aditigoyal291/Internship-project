...
def upsert_relationships(spark, dataframes):
    ...
    1. HAS_LOAN: Person to Loan
    2. WORKS_IN: Person to Company
    3. HAS_COAPPLICANT: Loan to Person (Coapplicant)
    4. HAS_REFERENCE: Loan to Person (Reference)
    """
    ...
    # 1. HAS_LOAN: Person to Loan (Primary Applicant)
...
def cleanup_isolated_nodes():
    ...
    try:
        ...
        DELETE n
        """
        graph.run(cleanup_query)
        logger.info("Cleaned up isolated nodes")
    except Exception as e:
        logger.error(f"Error cleaning up isolated nodes: {str(e)}")
    ...
    for node_type in node_types:
        if node_type in dataframes:
            ...
            for row in nodes:
                ...
                # Construct the appropriate query based on node type
                if node_type == 'person':
                    ...
                    # For Loan and Company nodes
                    label = node_type.capitalize()
                    id_key = f"{node_type}Id"
                    query = f"""
                    MERGE (n:{label} {{{id_key}: $nodeId}})
                    SET n += $properties
                    """
                ...
                try:
                    ...
                    logger.error(f"Error upserting {node_type.capitalize()} node: {str(e)}")
